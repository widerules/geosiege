...
import com.geosiege.common.collision.CollisionGrid;
...
public class CollisionGridTest extends TestCase {
  ...
  public void testAddStationaryObject() {
    ...
    assertEquals(object, grid.grid[0][0].objects[0]);
    assertEquals(object, grid.grid[1][0].objects[0]);
    assertEquals(object, grid.grid[1][1].objects[0]);
    assertEquals(object, grid.grid[0][1].objects[0]);
  }
  ...
  public void testAddObject() {
    ...
    assertEquals(object, grid.grid[0][0].objects[0]);
    assertEquals(0, grid.grid[1][0].numObjects);
    assertEquals(0, grid.grid[0][1].numObjects);
    assertEquals(0, grid.grid[1][1].numObjects);
  }
  ...
  public void testUpdatePosition() {
    ...
    assertEquals(0, grid.grid[0][0].numObjects);
    assertEquals(object, grid.grid[1][1].objects[0]);
  ...
  public void testGetObjectsAtWorldPosition() {
    ...
    assertEquals(object1, spot.objects[0]);
    assertEquals(object2, spot.objects[1]);
    
    spot = grid.getObjectsAtWorldPosition(101, 101);
    assertEquals(1, spot.numObjects);
    assertEquals(object3, spot.objects[0]);
  ...
  private boolean objectInGridSpots(GridSpot[] nearby, PhysicalObject object) {
    ...
    for ( int i = 0 ; i < nearby.length ; i++ ) {
      if (nearby[i] != null) {
        for (PhysicalObject temp : nearby[i].objects) {
          if (temp == object) {
            found = true;
          }
        }
